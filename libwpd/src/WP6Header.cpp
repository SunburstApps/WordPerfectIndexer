...
WP6Header::WP6Header(librevenge::RVNGInputStream *input, WPXEncryption *encryption, unsigned documentOffset, unsigned char productType,
...
{
	input->seek(WP6_HEADER_INDEX_HEADER_POINTER_OFFSET, librevenge::RVNG_SEEK_SET);
...
void WP6Header::_readIndexInformation(librevenge::RVNGInputStream *input, WPXEncryption *encryption)
{
	// read the Index Header (Header #0)
	// skip the Flags = 2 and the Reserved byte = 0
	input->seek(m_indexHeaderOffset + WP6_INDEX_HEADER_NUM_INDICES_POSITION, librevenge::RVNG_SEEK_SET);
	...
	input->seek(m_indexHeaderOffset + WP6_INDEX_HEADER_INDICES_POSITION, librevenge::RVNG_SEEK_SET);
}
